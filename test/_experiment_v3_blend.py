...
def compress_video_keyframes(
    ...
    *,
    ...
    def delete_score(i: int) -> float:
        ...
        quality_weight = 0.70 + 0.30 * blur_badness[i]
        l = prev_idx[i]
        rr = next_idx[i]
        left_gap = i - l if l is not None else 1
        right_gap = rr - i if rr is not None else 1
        balance = min(left_gap, right_gap) / max(left_gap, right_gap) if max(left_gap, right_gap) > 0 else 1.0
        target_gap = max(1.0, T / max(n, 1))
        span = left_gap + right_gap
        density_weight = 1.0 / (1.0 + (span / target_gap) ** 3)
        return float(r * quality_weight * (0.8 * density_weight + 0.2 * balance))
